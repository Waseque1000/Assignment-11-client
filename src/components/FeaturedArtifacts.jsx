...
const ArtifactCard = ({ artifact }) => {
  ...
  return (
    <div className="overflow-hidden rounded-lg bg-white dark:bg-gray-800 shadow-md dark:shadow-gray-700 transition-transform hover:-translate-y-1">
      <img
        src={artifact.image}
        alt={artifact.name}
        className="h-48 w-full object-cover"
      />
      <div className="p-4">
        <h3 className="mb-2 text-lg md:text-xl font-semibold text-gray-900 dark:text-gray-200">
          {artifact.name}
        </h3>
        <p className="mb-4 text-sm md:text-base text-gray-600 dark:text-gray-400 line-clamp-3">
          {artifact.description}
        </p>
        <div className="mb-4 flex flex-wrap gap-2 text-xs md:text-sm text-gray-500 dark:text-gray-300">
          <span className="flex items-center">
            <MdPlace className="mr-1 h-4 w-4" /> {artifact.location}
          </span>
          <span className="flex items-center">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={handleViewDetails}
            className="rounded-full bg-blue-600 dark:bg-blue-700 px-4 py-2 text-xs md:text-sm text-white hover:bg-blue-700 dark:hover:bg-blue-800 transition-colors"
          >
            View Details
          </button>
        </div>
...
const FeaturedArtifacts = () => {
  ...
  return (
    <section className="bg-gray-50 dark:bg-gray-900 py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <h2 className="mb-8 text-2xl md:text-3xl font-bold text-gray-900 dark:text-gray-200 text-center lg:text-left">
          Featured Artifacts
        </h2>
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {visibleArtifacts.map((artifact) => (
        ...
        {visibleCount < artifacts.length && (
          <div className="mt-8 flex justify-center">
            <button
              onClick={showAll}
              className="rounded-full bg-blue-600 dark:bg-blue-700 px-6 py-3 text-sm text-white hover:bg-blue-700 dark:hover:bg-blue-800 transition-colors"
            >
              Show All
            </button>
          </div>
